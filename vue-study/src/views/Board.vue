    ...
    <div ref="container">
        <v-container>
            ...
            <p>작성자 : <span class="ml-2">{{board.author}}</span>
                <span class="mx-5">|</span> 작성일 : <span class="ml-2">
                    {{board.createDate === undefined ? '0000-00-00' : board.createDate | moment('YYYY-MM-DD')}}
                </span>
            </p>
                        ...
                        <div :style="`max-width: ${getWidth(comment.depth)}%`" class="ma-3">
                            <div>
                                ...
                                <h4 class="d-inline-block">{{comment.name}} <span class="mx-1"/>작성일 :
                                    {{comment.createDate === undefined ? '0000-00-00' : comment.createDate |
                                    moment('YYYY-MM-DD')}}
                                </h4>
...
<script>
    ...
    export default {
        ...
        beforeDestroy() {
            ...
            this.$store.state.board.board.createDate = new Date();
        }
    }
</script>
